...
import com.chen.sequence.utils.RedisUtils;
...
public class CycRedisGenerate {
     ...
     * @return
     */
    public String redisSequenceFromCustom(String keyPrefix, long delTime, int seLength, int fillText, int beginText, int endText, int intervalNum){
        // 链接 Redis
        Jedis jedis = RedisUtils.getConnectJedis();
    ...
    public String getRedisSequence(SequenceBean sequenceBean){
        if ("TIME".equals(sequenceBean.getSequenceType())){
            return sequenceBean.getKeyPrefix()+redisSequenceFromTime(sequenceBean.getKeyPrefix(), sequenceBean.getDelTime());
        }else if ("ZERO".equals(sequenceBean.getSequenceType())){
            return sequenceBean.getKeyPrefix()+redisSequenceFromZero(sequenceBean.getKeyPrefix(),sequenceBean.getDelTime(),sequenceBean.getSeLength(),sequenceBean.getFillText());
        }else if ("CUSTOM".equals(sequenceBean.getSequenceType())){
            return sequenceBean.getKeyPrefix()+redisSequenceFromCustom(sequenceBean.getKeyPrefix(),sequenceBean.getDelTime(),sequenceBean.getSeLength(),sequenceBean.getFillText(),sequenceBean.getBeginText(),sequenceBean.getEndText(),sequenceBean.getIntervalNum());
        }
        return null;
    }
